...
export function tryAtHomeRescheduledTemplate(data: TryAtHomeRescheduledData) {
  ...
  const details = [['Appointment ID', data.appointmentId], ['New Date', formatEmailDate(data.newDate)],
    ['New Time', data.newTime], ['Delivery Address', data.deliveryAddress]];
  ...
  return {
    ...
    text: [`Dear ${name},`, '', 'Your Sunny Diamonds Try at Home appointment has been successfully rescheduled as requested.', '',
      'Updated Appointment Details', '', ...details.map(([label, value]) => `${label}: ${value}`), '',
      'Selected Jewellery', '', 'You have selected the following pieces for your Try at Home experience:', '',
      ...products.map(product => `- ${product}`), '',
      'We look forward to bringing your selected jewellery to you and creating a personalised experience from the comfort of your home.',
      ...manageText, '', 'Warm regards,', 'Team Sunny Diamonds',
...
<body style="margin:0;background:#f5f5f5;color:#222;font-family:Arial,sans-serif;line-height:1.6">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0"><tr><td align="center" style="padding:24px 12px">
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:600px;background:#fff"><tr><td style="padding:32px">
      ...
      <p>Dear ${escapeHtml(name)},</p><p>Your <strong>Sunny Diamonds Try at Home</strong> appointment has been successfully rescheduled as requested.</p>
      ...
      <p>You have selected the following pieces for your Try at Home experience:</p>
      <ul>${products.map(product => `<li><strong>${escapeHtml(product)}</strong></li>`).join('')}</ul>
      <p>We look forward to bringing your selected jewellery to you and creating a personalised experience from the comfort of your home.</p>
