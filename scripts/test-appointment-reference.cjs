...
const { buildAppointmentReference, assignAppointmentReference, backfillAppointmentReferences } = load('src/utils/appointment-reference.ts');
...
test('backfill assigns prefixes to historical groups and showroom visits', async () => {
  ...
  let query = 0;
  const strapi = { log: { info() {} }, db: { query: () => ({ findMany: async () => query++ === 0 ? groups : visits }) },
